...
def generate_dataset(word_corpus: List[str], output_dir: str = '.') -> None:
    ...
    for layout_name, layout in keyboard_layouts.items():
        ...
        # Create a QuantumKeyboard instance with the current layout
        # We need to modify the QuantumKeyboard class to accept a custom layout
        keyboard = QuantumKeyboard(layout_name)

        # Override the key positions with our comprehensive layout
        keyboard.key_positions = layout
        ...
        for i, word in enumerate(word_corpus):
            ...
            # Skip words with no valid positions
            if all(value == 0.0 for value in analysis['metrics'].values()):
                continue
