...
def decompress_path(compressed_path: bytes) -> List[Tuple[float, float, float]]:
    ...
    return json.loads(zlib.decompress(compressed_path).decode('utf-8'))

def gzip_compress_path(path: List[Tuple[float, float, float]]) -> bytes:
    """Compress a 3D path using gzip."""
    import io
    buffer = io.BytesIO()
...
def gzip_decompress_path(compressed_path: bytes) -> List[Tuple[float, float, float]]:
    """Decompress a 3D path using gzip."""
    import io
...
def main() -> None:
    ...
    # Compression example
    word = "hello"
    path = qwerty.get_3d_path(word)

    # Compress path
    compressed_path = compress_path(path)
    print(f"\nCompressed path size: {len(compressed_path)}")

    # Decompress path
    decompressed_path = decompress_path(compressed_path)
    print(f"Decompressed path: {decompressed_path}")

    # Gzip compress path
    gzip_compressed_path = gzip_compress_path(path)
    print(f"Gzip compressed path size: {len(gzip_compressed_path)}")

    # Gzip decompress path
    gzip_decompressed_path = gzip_decompress_path(gzip_compressed_path)
    print(f"Gzip decompressed path: {gzip_decompressed_path}")
